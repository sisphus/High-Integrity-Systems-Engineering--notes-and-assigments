...
package Universe with SPARK_Mode is
   ...
   procedure Init (U : out Universe)
     with Post =>
       Item_Count (U) = 0;
   --  After Init the universe is empty: item_count is 0 and no items can
   --  be retrieved (all getters require Index <= Item_Count, which is 0).
   ...
   procedure Add_Item
     ...
     with
       ...
       Post =>
         --  Exactly one new item is added at the new last index.
   ...
   procedure Tick (U : in out Universe)
     with Post =>
       ...
       --  Every item's position is advanced by its velocity; velocity and
